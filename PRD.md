...
### 已完成
- TCP JSON 行协议的局域网联机，主机启动服务器并自动连接。  
- 客户端手动输入房主 IP/端口连接。  
- 剧本加载与列表（`data/scripts/`）。  
- 主机选择剧本、设置人数（4–6）、随机分配角色。  
- 角色阅读界面与随时查看角色剧本（全阶段可读）。  
- 身份牌与公共介绍对所有玩家可见，改名同步到剧本与身份卡。  
- 搜证/投票/结果复盘基础界面与逻辑打通。  
- 投票匿名，结果公布前不展示票数分布。  
- 状态广播与玩家列表显示。  
- 示例剧本扩写，增加时间线、线索与推理链条。  

### 未完成
- 搜证/投票/结果复盘更复杂规则（多轮、限制、计时等）。  
- 存档/复盘加载。  
- 断线重连/错误提示的完善。  
- 测试和打包流程。  
